...
//	}
	...
	private void logFieldChange(Ticket ticket, String field, Object oldValue, Object newValue) {
		...
		if (oldValue instanceof User || newValue instanceof User) {
			
			if (oldValue instanceof User)
				oldValue = "\"" + UserSummary.from((User) oldValue).toString() + "\"";
			
			if (newValue instanceof User)
				newValue = "\"" + UserSummary.from((User) newValue).toString() + "\"";
			
		} else if (oldValue instanceof Instant || newValue instanceof Instant) {

			if (oldValue instanceof Instant)
				oldValue = "\"" + oldValue.toString() + "\""; // wrap in quotes
			
			if (newValue instanceof Instant)
		...
		TicketHistory history = new TicketHistory(ticket, field, oldValue, newValue, currentUser.getLoggedInUserId(),
				Instant.now());
